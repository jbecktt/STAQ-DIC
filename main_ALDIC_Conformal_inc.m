% ---------------------------------------------
% Augmented Lagrangian Digital Image Correlation (ALDIC_Conformal_inc)
% using an adaptive quadtree mesh, which was automatically generated
% based on the DIC raw images
% 
% Author: Joseph Beckett and Jin Yang, PhD @Caltech
% Contact and support: jbecktt@umich.edu -or- jyang526@wisc.edu -or- aldicdvc@gmail.com
% Date: 2015.04,06,07; 2016.03,04; 2020.11; 2024.1
% ---------------------------------------------
...
addpath("./func",'./src','./plotFiles','./func_quadtree','./func_quadtree/refinement','./plotFiles/export_fig-d966721/','./func_conformal'); 
addpath('./img_syn_circular_hole'); % TODO: addpath("./YOUR IMAGE FOLDER"); 
...
% ====== Read images ====== 
[file_name,Img,DICpara] = ReadImageQuadtree; % Load DIC raw images % JGB: it appears that incremental is turned off as of now also Subpb2FDOrFEM is set to 1 
try
...
for ImgSeqNum = 2 : length(ImgNormalized) 
    ...
    close all; 
    DICpara.NewFFTSearch = 1; % Apply the new FFT search since the incremental disp is small between two consecutive frames % JGB: apparent overwrite of ReadImageQuadtree.m
    ...
    Df = funImgGradient(fNormalized,fNormalized,fNormalizedMask); % Finite difference to compute image grayscale gradients; % JGB: why us 'splinecubic' section commented
    ...
    DICpara.InitFFTSearchMethod = 0; % JGB: Opts to do a multi-grid search until the grid smaller than the window size
